...
function domainPrefixer(config) {
    var re_abs_url = /("|'|=|url\(\s*)(https?:)\/\/((?:[A-Z0-9-]{1,63}\.){1,125}[A-Z]{2,63})/ig, // "http:, 'http:, =http:, or url( http:, also matches https versions
    ...
    function rewriteDomains(chunk, uri, prefix) {
